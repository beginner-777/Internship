...
export function getAssistantResponse(input) {
  ...
  if (/react.*project|project.*react/.test(query)) {
    const reactProjects = projects.filter((project) => project.stack.some((item) => /react/i.test(item)));
    if (!reactProjects.length) {
      return 'No verified React project is currently listed in Musfirah’s portfolio. Her documented client project uses HTML5, CSS3 and JavaScript, while React is listed as a foundational skill.';
    }
    return `Her verified React projects include ${reactProjects.map((project) => project.title).join(', ')}.`;
  }
