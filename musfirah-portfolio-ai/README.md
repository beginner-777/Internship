...
- Reference-inspired monochrome particle-ring boot sequence with a seamless home transition
- Persistent light and dark modes with a flash-free startup theme
- Interactive React Three Fiber neural core
...
- Tailwind CSS v4 design pipeline plus a custom Cosmic Carbon token system
...
The homepage automatically switches to a CSS-rendered AI core when an embedded browser does not expose WebGL. Storage access is also guarded, so VS Code preview panes and privacy-restricted browsers cannot collapse the interface into a blank page.
